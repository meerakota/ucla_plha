...
def get_im(vs30, rjb, m, fault_type):
    ...
    fe[m > mh] = (
        e1 * ss[m > mh] + e2 * ns[m > mh] + e3 * rs[m > mh] + e6 * (m[m > mh] - mh)
    )

    # path function
    r = np.sqrt(rjb**2 + h**2)  # Equation 4
    fpath = (c1 + c2 * (m - mref)) * np.log(r / rref) + (c3 + deltac3) * (
        r - rref
    )  # Equation 3
    ...
    f2 = f4 * (
        np.exp(f5 * (min(vs30, vref) - 360)) - np.exp(f5 * (vref - 360))
    )  # Equation 8
    ...
    lnfs = (
        lnflin + lnfnl
    )  # equation 5 (skipping Fdz term since it doesn't influence PGA)
    ...
    phi_m_rjb[(r1 < rjb) & (rjb <= r2)] = phi_m[
        (r1 < rjb) & (rjb <= r2)
    ] + deltaphir * np.log(rjb[(r1 < rjb) & (rjb <= r2)] / r1) / np.log(r2 / r1)
    ...
    phi[(v1 < vs30) & (vs30 < v2)] = phi_m_rjb[
        (v1 < vs30) & (vs30 < v2)
    ] - deltaphiv * np.log(v2 / vs30) / np.log(v2 / v1)
