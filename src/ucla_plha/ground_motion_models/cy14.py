...
def get_im(vs30, rjb, rrup, rx, m, fault_type, measured_vs30, dip, ztor, **kwargs):
    ...
    if n_check is not None:
        ...
        z1p0_ref = np.exp(
            -7.15
            / 4.0
            * np.log((1130.0**4.0 + 571.0**4.0) / (1360.0**4.0 + 571.0**4.0))
        )
    ...
    lnyrefij[m > 4.5] += (c1b + c1d / (np.cosh(2.0 * (m[m > 4.5] - 4.5)))) * fnm[
        m > 4.5
    ]
    lnyrefij[m <= 4.5] += (c1b + c1d) * fnm[m <= 4.5]

    lnyrefij[m > 4.5] += (c7 + c7b / (np.cosh(2.0 * (m[m > 4.5] - 4.5)))) * delta_ztor[
        m > 4.5
    ]
    lnyrefij[m <= 4.5] += (c7 + c7b / np.cosh(2.0 * 0)) * delta_ztor[m <= 4.5]

    lnyrefij[m > 4.5] += (c11 + c11b / (np.cosh(2.0 * (m[m > 4.5] - 4.5)))) * np.cos(
        dip[m > 4.5] * np.pi / 180.0
    ) ** 2
    lnyrefij[m <= 4.5] += (c11 + c11b) * np.cos(dip[m <= 4.5] * np.pi / 180.0) ** 2

    lnyrefij += c2 * (m - 6.0) + (c2 - c3) / cn * np.log(1.0 + np.exp(cn * (cm - m)))

    lnyrefij[m > chm] += c4 * np.log(
        rrup[m > chm] + c5 * np.cosh(c6 * (m[m > chm] - chm))
    )
    ...
    lnyij += (
        phi2
        * (
            np.exp(phi3 * (np.minimum(vs30, 1130.0) - 360.0))
            - np.exp(phi3 * (1130.0 - 360.0))
        )
        * np.log((yrefij + phi4) / phi4)
    )
    ...
    nl0 = (
        phi2
        * (np.exp(phi3 * (vs30_lt_1130 - 360.0)) - np.exp(phi3 * (1130.0 - 360.0)))
        * (yrefij / (yrefij + phi4))
    )
    if measured_vs30:
        finferred = 0.0
        ...
        finferred = 1.0
        fmeasured = 0.0
    sigmanl0 = (sigma1 + (sigma2 - sigma1) / 1.5 * (ml6p5 - 5.0)) * np.sqrt(
        sigma3 * finferred + 0.7 * fmeasured + (1 + nl0) ** 2
    )
