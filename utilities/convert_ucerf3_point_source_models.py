...
def get_points(
    points_input_filename,
    weights_input_filename,
    points_output_filename,
    node_index_output_filename,
    ruptures_output_filename,
):
    """
    ...
    for wni in weight_node_index:
        if wni in unique_node_index:
            ...
            filter.append(False)
    rate_ss = (
        np.repeat(df_weights["Fraction Strike-Slip"].values[filter], len(m)) * rates
    )
